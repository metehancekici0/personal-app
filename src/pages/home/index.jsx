...
export default function Home() {
  useEffect(() => {
    ...
    const loadScripts = async () => {
      try {
        removeHomeScript = await addScript(process.env.REACT_APP_BASE_URL + "assets/js/typing.js");
        ...
        removeBgAnimationScript = await addScript(process.env.REACT_APP_BASE_URL + "assets/js/bgAnimation.js");
      } catch (error) {
    ...
    return () => {
      if (removeHomeScript) removeHomeScript();
      if (removeTweenScript) removeTweenScript();
      if (removePluginScript) removePluginScript();
      if (removeBgAnimationScript) removeBgAnimationScript();
    };
  }, []);
  return (
    <>
      <Helmet>
        <title>Metehan Çekici - Personal website</title>
      ...
      <section className="bannerPart">
        <h1 className="name glitch" data-text="Metehan ÇEKİCİ">
          Metehan ÇEKİCİ
        </h1>
        <div className="hero-skills"></div>
        <div className="socials">
          <Link to="https://github.com/metehancekici0" target="_blank">
            <FaGithub />
          </Link>
          <Link to="https://www.linkedin.com/in/metehan-cekici/" target="_blank">
            <FaLinkedinIn />
          </Link>
          <Link to="https://mail.google.com/mail/?view=cm&fs=1&tf=1&to=metehancekici0@gmail.com" target="_blank">
            <FaEnvelope />
          </Link>
        </div>
        <svg
